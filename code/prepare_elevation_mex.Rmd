...
mapview(srtm_crop, map.types = mapviewGetOption("basemaps")[c(3,4,1,2,5)])
```
...
# reorder basemaps so that OpenStreetmap is first
mapview(srtm_focal, map.types = mapviewGetOption("basemaps")[c(3,4,1,2,5)])
```
...
mapview(grid_aoi, zcol = "elev_filtered", map.types = mapviewGetOption("basemaps")[c(3,4,1,2,5)])
```
...
#### Line plots, sorted by increasing filtered elevation values:  

```{r line_extracted}
ggplot(grid_melt, aes(index, Elevation_Meters, group=Extract_Method)) + geom_line(aes(color=Extract_Method))
```
