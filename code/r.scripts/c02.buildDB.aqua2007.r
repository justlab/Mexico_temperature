...
aqua.2007 <- readRDS("./data/RAW/MODIS.AQUA.TERRA.LST.NDVI/stage2/MYD11A1_2007.rds")
a.2007 <- subset(aqua.2007, lstid %in% fullgrid$lstid)
a.2007 <- as.data.table(a.2007)
# Create full LU TS
days <- seq.Date(from = as.Date("2007-01-01"), to = as.Date("2007-12-31"), 1)
# Create date range
days2007 <- data.table(expand.grid(lstid = fullgrid[, unique(lstid)], day = days))
# days2007$lstid <- as.character(days2007$lstid)
# Merge 
setkey(a.2007, lstid, day)
setkey(days2007,lstid, day)
db2007 <- merge(days2007, a.2007, all.x = T)
...
db2007 <- merge(db2007, fullgrid, all.x = T)  
gc()
head(db2007)
...
# Join NDVI to LST.
fin.ndvi <- readRDS("./data/RAW/MODIS.AQUA.TERRA.LST.NDVI/stage2/MYD13A3_2007.rds")
fin.ndvi <- as.data.table(fin.ndvi)
fin.ndvi[, m := as.numeric(format(day, "%m"))]
# fin.ndvi <- filter(fin.ndvi, c==2007)
...
db2007 <- merge(db2007, f.ndvi[,list(ndviid,ndvi,m)], all.x = T)
db2007 <- db2007[complete.cases(db2007$lat_lst),]
gc()
summary(db2007)
...
Temp2007 <- filter(Temp, c==2007)
temp2007tc <- select(Temp2007, stn, r.humidity.mean, lat_stn=Y, long_stn=X, day)
temp2007tc <- na.omit(temp2007tc)
temp2007tc <- as.data.table(temp2007tc)
temp2007tc$stn <- as.character(temp2007tc$stn)

# Spatiotemporal join. 
# Create point matrices for temperature. 
met.m <- makepointsmatrix(temp2007tc, "long_stn", "lat_stn", "stn")
setkey(db2007, lstid)
lu.m <- makepointsmatrix(db2007[db2007[,unique(lstid)], list(long_lst, lat_lst, lstid), mult = "first"], "long_lst", "lat_lst", "lstid")
...
closestaodse <- nearestbyday(lu.m, met.m, 
                             db2007, temp2007tc[, list(day,r.humidity.mean,stn)], 
                             "lstid", "stn", "meanT", "r.humidity.mean", knearest = 7, maxdistance = 50000)


setkey(db2007, lstid, day)
setkey(closestaodse, lstid, day)
db2007 <- merge(db2007, closestaodse[,list(day, r.humidity.mean, lstid)], all.x = T)

# Wind speed mean
Temp2007 <- filter(Temp,c==2007)
temp2007tc <- select(Temp2007,stn,wind.speed.mean,lat_stn= Y ,long_stn= X,day)
temp2007tc <- na.omit(temp2007tc)
temp2007tc <- as.data.table(temp2007tc)
temp2007tc$stn <- as.character(temp2007tc$stn)
...
closestaodse <- nearestbyday(lu.m ,met.m , 
                             db2007, temp2007tc[, list(day, wind.speed.mean, stn)], 
                             "lstid", "stn", "meanT", "wind.speed.mean", knearest = 7, maxdistance = 50000)


setkey(db2007, lstid, day)
setkey(closestaodse, lstid, day)
db2007 <- merge(db2007, closestaodse[,list(day, wind.speed.mean, lstid)], all.x = T)

# Bar (barometric pressure?) mean
Temp2007 <- filter(Temp, c==2007)
temp2007tc <- select(Temp2007, stn, bar.mean, lat_stn=Y, long_stn=X, day)
temp2007tc <- na.omit(temp2007tc)
temp2007tc <- as.data.table(temp2007tc)
temp2007tc$stn <- as.character(temp2007tc$stn)
...
# Use nearestbyday function to find nearest met station by day with wind speed data.
closestaodse<- nearestbyday(lu.m ,met.m , 
                            db2007, temp2007tc[, list(day,bar.mean,stn)], 
                            "lstid", "stn", "meanT", "bar.mean", knearest = 7, maxdistance = 50000)


setkey(db2007, lstid, day)
setkey(closestaodse, lstid, day)
db2007 <- merge(db2007, closestaodse[,list(day, bar.mean, lstid)], all.x = T)


# Rain mean.
Temp2007 <- filter(Temp, c==2007)
temp2007tc <- select(Temp2007, stn, rain.mean, lat_stn=Y, long_stn=X, day)
temp2007tc <- na.omit(temp2007tc)
temp2007tc <- as.data.table(temp2007tc)
temp2007tc$stn <- as.character(temp2007tc$stn)
...
closestaodse<- nearestbyday(lu.m ,met.m , 
                            db2007, temp2007tc[, list(day,rain.mean,stn)], 
                            "lstid", "stn", "meanT", "rain.mean", knearest = 7, maxdistance = 50000)


setkey(db2007, lstid, day)
setkey(closestaodse, lstid, day)
db2007 <- merge(db2007, closestaodse[,list(day, rain.mean, lstid)], all.x = T)
...
Temp[, day:=as.Date(strptime(date, "%Y-%m-%d"))]
Temp[, c := as.numeric(format(day, "%Y"))]
Temp2007tc <- as.data.frame(Temp)
Temp2007tc <- select(Temp2007tc, stn, day,hi.temp, low.temp, temp.mean, long_stn=X, lat_stn=Y)



#loop min
for(i in unique(db2007$day)) {
  x<-Temp2007tc[Temp2007tc$day==i, ]
  y= db2007[db2007$day==i, ]
  ##########
  # calculate IDW
  library(gstat)
  ...
  # head(z)
  db2007$predmin[db2007$day==i] = z$var1.pred
...
for(i in unique(db2007$day)) {
  ...
  z<-predict(object = inter, newdata = y)
  # head(z)
  db2007$predmean[db2007$day==i] = z$var1.pred
  # spplot(z, "var1.pred", at = 0:330)
}
...
for(i in unique(db2007$day)) {
  ...
  z<-predict(object = inter, newdata = y)
  # head(z)
  db2007$predmax[db2007$day==i] = z$var1.pred
  # spplot(z, "var1.pred", at = 0:330)
}
...
saveRDS(db2007,"./data/outputs/AQUA/2007/c02/MEXICO.mod3.AQ.2007.rds")
gc()
...
#create mod 2 file
db2007.m2.day <- db2007[!is.na(d.tempc)]
db2007.m2.night <- db2007[!is.na(n.tempc)]
#rm m3
rm(db2007)
gc()
#save mod2

saveRDS(db2007.m2.day,"./data/outputs/AQUA/2007/c02/MEXICO.mod2.AQ.2007.day.rds")
saveRDS(db2007.m2.night,"./data/outputs/AQUA/2007/c02/MEXICO.mod2.AQ.2007.night.rds")
...
db2007days <- sort(unique(db2007.m2.night$day))
...
Temp[, day:=as.Date(strptime(date, "%Y-%m-%d"))]
Temp[, c := as.numeric(format(day, "%Y"))]
Temp2007 <- select(Temp, stn, day, hi.temp, low.temp, temp.mean, c, long_stn=X, lat_stn=Y)
Ta <- filter(Temp2007, c==2007)
Ta <- as.data.table(Ta)
...
setkey(db2007.m2.night, lstid)
lst.m <- makepointsmatrix(db2007.m2.night[db2007.m2.night[,unique(lstid)], list(long_lst, lat_lst, lstid), mult = "first"], "long_lst", "lat_lst", "lstid")

#run function
closestlst <- nearestbyday(Ta.m, lst.m, 
                           Ta[day %in% db2007days,], db2007.m2.night, 
...
#save mod 1
saveRDS(Ta.m1,"./data/outputs/AQUA/2007/c02/MEXICO.mod1.AQ.2007.night.rds")



########--------->mod1 day
#to fix missing days issues resulting in cartesean error
db2007days <- sort(unique(db2007.m2.day$day))
...
Ta.m <- makepointsmatrix(Ta, "long_stn", "lat_stn", "stn")
#create lst terra matrix
setkey(db2007.m2.day, lstid)
lst.m <- makepointsmatrix(db2007.m2.day[db2007.m2.day[,unique(lstid)], list(long_lst, lat_lst, lstid), mult = "first"], "long_lst", "lat_lst", "lstid")

#run function
closestlst <- nearestbyday(Ta.m, lst.m, 
                           Ta[day %in% db2007days,], db2007.m2.day, 
...
#save mod 1
saveRDS(Ta.m1,"./data/outputs/AQUA/2007/c02/MEXICO.mod1.AQ.2007.day.rds")
...
Ta.m1 <- Ta.m1[!is.na(n.tempc)]
#save mod 1
saveRDS(Ta.m1,"./data/outputs/AQUA/2007/c02/MEXICO.mod1.AQ.2007.mean24.rds")
# ########--------->mod1 mean
# #to fix missing days issues resulting in cartesean error
