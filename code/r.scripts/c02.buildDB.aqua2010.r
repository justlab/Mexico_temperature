...
aqua.2010 <- readRDS("./data/RAW/MODIS.AQUA.TERRA.LST.NDVI/stage2/MYD11A1_2010.rds")
a.2010 <- subset(aqua.2010, lstid %in% fullgrid$lstid)
a.2010 <- as.data.table(a.2010)
# Create full LU TS
days <- seq.Date(from = as.Date("2010-01-01"), to = as.Date("2010-12-31"), 1)
# Create date range
days2010 <- data.table(expand.grid(lstid = fullgrid[, unique(lstid)], day = days))
# days2010$lstid <- as.character(days2010$lstid)
# Merge 
setkey(a.2010, lstid, day)
setkey(days2010,lstid, day)
db2010 <- merge(days2010, a.2010, all.x = T)
...
db2010 <- merge(db2010, fullgrid, all.x = T)  
gc()
head(db2010)
...
# Join NDVI to LST.
fin.ndvi <- readRDS("./data/RAW/MODIS.AQUA.TERRA.LST.NDVI/stage2/MYD13A3_2010.rds")
fin.ndvi <- as.data.table(fin.ndvi)
fin.ndvi[, m := as.numeric(format(day, "%m"))]
# fin.ndvi <- filter(fin.ndvi, c==2010)
...
db2010 <- merge(db2010, f.ndvi[,list(ndviid,ndvi,m)], all.x = T)
db2010 <- db2010[complete.cases(db2010$lat_lst),]
gc()
summary(db2010)
...
Temp2010 <- filter(Temp, c==2010)
temp2010tc <- select(Temp2010, stn, r.humidity.mean, lat_stn=Y, long_stn=X, day)
temp2010tc <- na.omit(temp2010tc)
temp2010tc <- as.data.table(temp2010tc)
temp2010tc$stn <- as.character(temp2010tc$stn)

# Spatiotemporal join. 
# Create point matrices for temperature. 
met.m <- makepointsmatrix(temp2010tc, "long_stn", "lat_stn", "stn")
setkey(db2010, lstid)
lu.m <- makepointsmatrix(db2010[db2010[,unique(lstid)], list(long_lst, lat_lst, lstid), mult = "first"], "long_lst", "lat_lst", "lstid")
...
closestaodse <- nearestbyday(lu.m, met.m, 
                             db2010, temp2010tc[, list(day,r.humidity.mean,stn)], 
                             "lstid", "stn", "meanT", "r.humidity.mean", knearest = 7, maxdistance = 50000)


setkey(db2010, lstid, day)
setkey(closestaodse, lstid, day)
db2010 <- merge(db2010, closestaodse[,list(day, r.humidity.mean, lstid)], all.x = T)

# Wind speed mean
Temp2010 <- filter(Temp,c==2010)
temp2010tc <- select(Temp2010,stn,wind.speed.mean,lat_stn= Y ,long_stn= X,day)
temp2010tc <- na.omit(temp2010tc)
temp2010tc <- as.data.table(temp2010tc)
temp2010tc$stn <- as.character(temp2010tc$stn)
...
closestaodse <- nearestbyday(lu.m ,met.m , 
                             db2010, temp2010tc[, list(day, wind.speed.mean, stn)], 
                             "lstid", "stn", "meanT", "wind.speed.mean", knearest = 7, maxdistance = 50000)


setkey(db2010, lstid, day)
setkey(closestaodse, lstid, day)
db2010 <- merge(db2010, closestaodse[,list(day, wind.speed.mean, lstid)], all.x = T)

# Bar (barometric pressure?) mean
Temp2010 <- filter(Temp, c==2010)
temp2010tc <- select(Temp2010, stn, bar.mean, lat_stn=Y, long_stn=X, day)
temp2010tc <- na.omit(temp2010tc)
temp2010tc <- as.data.table(temp2010tc)
temp2010tc$stn <- as.character(temp2010tc$stn)
...
# Use nearestbyday function to find nearest met station by day with wind speed data.
closestaodse<- nearestbyday(lu.m ,met.m , 
                            db2010, temp2010tc[, list(day,bar.mean,stn)], 
                            "lstid", "stn", "meanT", "bar.mean", knearest = 7, maxdistance = 50000)


setkey(db2010, lstid, day)
setkey(closestaodse, lstid, day)
db2010 <- merge(db2010, closestaodse[,list(day, bar.mean, lstid)], all.x = T)


# Rain mean.
Temp2010 <- filter(Temp, c==2010)
temp2010tc <- select(Temp2010, stn, rain.mean, lat_stn=Y, long_stn=X, day)
temp2010tc <- na.omit(temp2010tc)
temp2010tc <- as.data.table(temp2010tc)
temp2010tc$stn <- as.character(temp2010tc$stn)
...
closestaodse<- nearestbyday(lu.m ,met.m , 
                            db2010, temp2010tc[, list(day,rain.mean,stn)], 
                            "lstid", "stn", "meanT", "rain.mean", knearest = 7, maxdistance = 50000)


setkey(db2010, lstid, day)
setkey(closestaodse, lstid, day)
db2010 <- merge(db2010, closestaodse[,list(day, rain.mean, lstid)], all.x = T)
...
Temp[, day:=as.Date(strptime(date, "%Y-%m-%d"))]
Temp[, c := as.numeric(format(day, "%Y"))]
Temp2010tc <- as.data.frame(Temp)
Temp2010tc <- select(Temp2010tc, stn, day,hi.temp, low.temp, temp.mean, long_stn=X, lat_stn=Y)



#loop min
for(i in unique(db2010$day)) {
  x<-Temp2010tc[Temp2010tc$day==i, ]
  y= db2010[db2010$day==i, ]
  ##########
  # calculate IDW
  library(gstat)
  ...
  # head(z)
  db2010$predmin[db2010$day==i] = z$var1.pred
...
for(i in unique(db2010$day)) {
  ...
  z<-predict(object = inter, newdata = y)
  # head(z)
  db2010$predmean[db2010$day==i] = z$var1.pred
  # spplot(z, "var1.pred", at = 0:330)
}
...
for(i in unique(db2010$day)) {
  ...
  z<-predict(object = inter, newdata = y)
  # head(z)
  db2010$predmax[db2010$day==i] = z$var1.pred
  # spplot(z, "var1.pred", at = 0:330)
}
...
saveRDS(db2010,"./data/outputs/AQUA/2010/c02/MEXICO.mod3.AQ.2010.rds")
gc()
...
#create mod 2 file
db2010.m2.day <- db2010[!is.na(d.tempc)]
db2010.m2.night <- db2010[!is.na(n.tempc)]
#rm m3
rm(db2010)
gc()
#save mod2

saveRDS(db2010.m2.day,"./data/outputs/AQUA/2010/c02/MEXICO.mod2.AQ.2010.day.rds")
saveRDS(db2010.m2.night,"./data/outputs/AQUA/2010/c02/MEXICO.mod2.AQ.2010.night.rds")
...
db2010days <- sort(unique(db2010.m2.night$day))
...
Temp[, day:=as.Date(strptime(date, "%Y-%m-%d"))]
Temp[, c := as.numeric(format(day, "%Y"))]
Temp2010 <- select(Temp, stn, day, hi.temp, low.temp, temp.mean, c, long_stn=X, lat_stn=Y)
Ta <- filter(Temp2010, c==2010)
Ta <- as.data.table(Ta)
...
setkey(db2010.m2.night, lstid)
lst.m <- makepointsmatrix(db2010.m2.night[db2010.m2.night[,unique(lstid)], list(long_lst, lat_lst, lstid), mult = "first"], "long_lst", "lat_lst", "lstid")

#run function
closestlst <- nearestbyday(Ta.m, lst.m, 
                           Ta[day %in% db2010days,], db2010.m2.night, 
...
#save mod 1
saveRDS(Ta.m1,"./data/outputs/AQUA/2010/c02/MEXICO.mod1.AQ.2010.night.rds")



########--------->mod1 day
#to fix missing days issues resulting in cartesean error
db2010days <- sort(unique(db2010.m2.day$day))
...
Ta.m <- makepointsmatrix(Ta, "long_stn", "lat_stn", "stn")
#create lst terra matrix
setkey(db2010.m2.day, lstid)
lst.m <- makepointsmatrix(db2010.m2.day[db2010.m2.day[,unique(lstid)], list(long_lst, lat_lst, lstid), mult = "first"], "long_lst", "lat_lst", "lstid")

#run function
closestlst <- nearestbyday(Ta.m, lst.m, 
                           Ta[day %in% db2010days,], db2010.m2.day, 
...
#save mod 1
saveRDS(Ta.m1,"./data/outputs/AQUA/2010/c02/MEXICO.mod1.AQ.2010.day.rds")
...
Ta.m1 <- Ta.m1[!is.na(n.tempc)]
#save mod 1
saveRDS(Ta.m1,"./data/outputs/AQUA/2010/c02/MEXICO.mod1.AQ.2010.mean24.rds")
# ########--------->mod1 mean
# #to fix missing days issues resulting in cartesean error
